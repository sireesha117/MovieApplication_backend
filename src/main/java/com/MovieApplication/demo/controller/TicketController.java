...
//			ticket.setTheatreName(b1.getTheatreName()); 
...
//	}
	...
	public ResponseEntity<?> addTicket(@PathVariable("mid") int movieId, @RequestBody Ticket ticket) {
	    ...
	    if (movie != null) {
	        // Check if the number of tickets is less than or equal to the available seats
	        if (ticket.getNo_of_tickets() <= movie.getSeatsAvalible()) {
	            movie.setSeatsAvalible(movie.getSeatsAvalible() - ticket.getNo_of_tickets());
	            ticket.setMovie_id_fk(ticket.getMovie_id_fk());
	            ticket.setUser_name_fk(ticket.getUser_name_fk());
	            ...
	            ticket.setTotalPrice(movie.getPrice() * ticket.getNo_of_tickets());

	            if (ms.updateMovie(movie) && ts.addTicket(ticket)) {
	                return new ResponseEntity<Ticket>(ticket, HttpStatus.CREATED);
	            }
	        } else {
	            return new ResponseEntity<String>("Not enough seats available", HttpStatus.BAD_REQUEST);
	        }
	    }
	    return new ResponseEntity<String>("Ticket cannot be booked", HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	@GetMapping("/findticket/{userName}")
